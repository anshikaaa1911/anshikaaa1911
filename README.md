<h1 align="center">Hi &#128075;, I'm Anshika</h1>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=24&pause=1000&color=00C4FF&center=true&vCenter=true&width=600&lines=AI+%26+Data+Science+Student;Full+Stack+Developer;Generative+AI+Enthusiast;Agentic+AI+Developer;DSA+in+C%2B%2B" alt="Typing animation: AI and Data Science Student, Full Stack Developer, Generative AI Enthusiast, Agentic AI Developer, DSA in C++" />
</p>

<div align="center">

### Building ML-powered products, full-stack apps, and data-driven systems.

[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-0A66C2?style=for-the-badge&logo=githubpages&logoColor=white)](https://anshikaaa1911.github.io/Portfolio/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/anshika-bharti)
[![Email](https://img.shields.io/badge/Email-Say%20Hello-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:anshikabharti1911@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-anshikaaa1911-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/anshikaaa1911)

</div>

---

## What I'm Building

I work at the intersection of **machine learning**, **data science**, **cybersecurity awareness**, and **full-stack development**. I like turning messy real-world problems into useful systems: fraud simulators, recommendation engines, analytics workflows, and portals that people can actually use.

```text
Current focus:
Machine Learning      Fraud Detection      Recommendation Systems
Computer Vision       MERN Applications    Data Analytics
```

## Snapshot

| Area | Details |
| --- | --- |
| Education | B.Tech CSE (AI & Data Science), Jaypee University of Information Technology, 2023-2027 |
| Experience | Engineering Intern at RepCard Private Limited, Gurugram |
| Strengths | Python, ML pipelines, statistics, full-stack development, analytics |
| Interests | Cyber fraud detection, deepfake analysis, recommendation systems, data-driven products |

## Featured Work

<table>
  <tr>
    <td width="50%">
      <h3>Behavioral Fraud Awareness & Simulation Platform</h3>
      <p>Interactive cyber fraud simulation platform with deep learning-based scam classification.</p>
      <ul>
        <li>CNN and XceptionNet-based classification</li>
        <li>Assistant module for awareness and prevention</li>
        <li>Research on cyber fraud, deepfakes, and ML-based detection</li>
      </ul>
    </td>
    <td width="50%">
      <h3>College Marketplace + ML Recommendations</h3>
      <p>Peer-to-peer resale platform for students with a behavior-based recommendation system.</p>
      <ul>
        <li>User interaction and behavioral data modeling</li>
        <li>Preprocessing and feature engineering</li>
        <li>Improved product discovery using ML</li>
      </ul>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3>Employee Goal Management Portal</h3>
      <p>MERN-based portal for goal setting, approvals, performance tracking, reporting, and admin workflows.</p>
      <ul>
        <li>JWT authentication and role-based access</li>
        <li>REST APIs for goals, teams, users, audit logs, and exports</li>
        <li>MongoDB models with Mongoose</li>
      </ul>
    </td>
    <td width="50%">
      <h3>GROWNET Trust-Based Community Platform</h3>
      <p>Community platform concept focused on trust scoring, SOS alerts, and recommendation features.</p>
      <ul>
        <li>Trust-based interaction design</li>
        <li>Secure and reliable community workflows</li>
        <li>Data-driven recommendation features</li>
      </ul>
    </td>
  </tr>
</table>

## Tech Stack

### Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![C](https://img.shields.io/badge/C-A8B9CC?style=flat-square&logo=c&logoColor=black)

### Data Science & Machine Learning

![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white)

### Development

![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)

## Research & Learning

<details>
<summary><b>Research: Behavioral Fraud Awareness & Simulation Platform</b></summary>

- Authored and presented a conference paper on cyber fraud detection, deepfake analysis, and ML-based scam classification.
- Reviewed fraud detection algorithms, cybersecurity regulations, and emerging ML security solutions.
- Explored how awareness tools can make complex fraud patterns easier for users to recognize.

</details>

<details>
<summary><b>Certifications</b></summary>

- Functional Data Structures and Algorithms
- Deloitte Data Analytics Virtual Experience Program
- Agentic AI and Generative AI Applications

</details>

<details>
<summary><b>Professional Strengths</b></summary>

- Analytical thinking and problem solving
- Technical communication and stakeholder collaboration
- Teamwork, adaptability, and leadership in project development
- Ability to interpret data-driven insights for decision making

</details>

## GitHub Activity

<div align="center">

![Anshika's GitHub stats](https://github-readme-stats.vercel.app/api?username=anshikaaa1911&show_icons=true&theme=tokyonight&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=anshikaaa1911&layout=compact&theme=tokyonight&hide_border=true)

</div>

## Let's Connect

I'm open to internships, collaborations, research-focused builds, and product ideas involving machine learning, data analytics, and full-stack development.

<div align="center">

[Portfolio](https://anshikaaa1911.github.io/Portfolio/) | [LinkedIn](https://linkedin.com/in/anshika-bharti) | [GitHub](https://github.com/anshikaaa1911) | [Email](mailto:anshikabharti1911@gmail.com)

</div>
